...
test.describe.serial('Resource Category CRUD Operations', () => {
  ...
  test('should display the created resource category', async ({ resourceCategoryPage, page }) => {
    ...
    await expect(page.locator('h1')).toContainText('E2E Test Resource Category');
  });
  ...
  test('should update the resource category', async ({ resourceCategoryPage, page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Updated E2E Resource Category');
  });
...
test.describe('Resource Category Custom Template Rendering', () => {
  ...
  test('should render category name and CustomSidebar in lightbox edit drawer', async ({ page }) => {
    ...
    const lightbox = page.getByRole('dialog', { name: `RC Template Test Image ${testRunId}`, exact: true });
  ...
  test('should show default category in lightbox for resources created without explicit category', async ({ apiClient, page }) => {
    ...
    try {
      ...
      const lightbox = page.getByRole('dialog', { name: `Default Category Resource ${testRunId}`, exact: true });
      await expect(lightbox).toBeVisible();
