...
test.describe('Bug 3: Admin overview should use correct singular/plural nouns', () => {
  ...
  test('admin overview should say "1 group" not "1 groups" for single-count categories', async ({
    ...
    // Wait for Alpine.js to hydrate the "Top Categories" list before asserting on
    // its text, otherwise the negative assertion below passes vacuously.
    const categoryRows = detailedStats.locator('li', {
      has: page.locator('a[href*="/category?id="]'),
    });
    await expect(categoryRows.first()).toBeVisible({ timeout: 30000 });

    // The regression: a count of 1 must never render as "1 groups". This holds
    // for every listed category, so it does not depend on which ones are listed.
    await expect(
      detailedStats.getByText('1 groups', { exact: true })
    ).toHaveCount(0);

    // "Top Categories" is LIMIT 10 ordered by group count descending (see
    // ctx.AdminStats in application_context/admin_context.go), so the category
    // seeded above -- which has exactly one group -- is listed only while fewer
    // than ten categories have any groups at all. Waiting for it to appear made
    // this test depend on how much data earlier specs had created; assert the
    // singular form when the row is present instead.
    const seededRow = detailedStats.locator('li', {
      has: page.locator(`a[href*="/category?id=${categoryId}"]`),
    });
    if (await seededRow.count()) {
      await expect(seededRow.getByText('1 group', { exact: true })).toBeVisible();
    }
  });
