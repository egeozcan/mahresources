---
outputShape: Array of schedule objects with scheduleId, pluginName, everySeconds, overlap, nextDueAt, runs, lastStatus, lastError, owned, registered, and lastRunAt once it has run, in JSON mode; a table in human mode whose STATE column collapses owned and registered into active, stopped (no owner) or not declared
