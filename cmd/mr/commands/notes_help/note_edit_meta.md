...
relatedCmds: note get, notes add-meta, notes meta-keys
---
...
# Example
  ...
  # mr-doctest: create, set meta, verify
  ID=$(mr note create --name "doctest-editmeta-$$-$RANDOM" --json | jq -r '.ID')
  mr note edit-meta $ID priority 5
  mr note get $ID --json | jq -e '.Meta.priority == 5'
