...
Fetch the log entries recorded for one specific entity. Both
...
`totalCount` reports how many entries exist. The command sends no page
parameter, so it returns the newest 50 entries and older pages are not
...
`reset`, `plugin`, `progress`) lets scripts filter to just the events
they care about.
