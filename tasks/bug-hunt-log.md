...
- **Status:** **FIXED** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a — see Fixed / closed table below)
- **Original status (pre-fix):** verified (iter 13, orchestrator re-confirmed via curl of `/notes?shared=true` — three share tokens in the page body)
...
- **Status:** **FIXED** (2026-04-22, c18-obs-search-docs, PR #41 merged cd4f0bf3 — see Fixed / closed table below)
...
- **Status:** **FIXED** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a — see Fixed / closed table below)
- **Original status (pre-fix):** verified (iter 12)
...
- **Status:** **FIXED** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a — see Fixed / closed table below). Share server ships the full middleware set (XFO, nosniff, Referer, strict CSP, HSTS); primary server ships the CSP-free subset with a tighter primary CSP tracked as a follow-up.
- **Original status (pre-fix):** verified (iter 12)
...
- **Status:** **FIXED** (2026-04-22, c17-a11y-batch-3, PR #39 merged 5270e8b1 — see Fixed / closed table below)
...
- **Status:** **FIXED** (2026-04-22, c16-group-ux, PR #43 merged 38a3d012 — see Fixed / closed table below)
...
- **Status:** **split** (2026-04-22, c18-obs-search-docs, PR #41 merged cd4f0bf3) — **BH-005a fixed** (case-insensitive LIKE fallback + fuzzy fallback on SQLite); **BH-005b deferred** (typo tolerance + Unicode case-folding on LIKE fallback — new entry at top of Active section, needs brainstorm).
...
| BH-010 | **fixed** (2026-04-22, c15-schema-block-editor, PR #38 merged 22a227c1) | New `getPreviewDefaultValue(schema, rootSchema)` in `src/schema-editor/schema-core.ts` mirrors `getDefaultValue` but returns `undefined` for number/integer/string with no explicit `default` (and prefers `null` for nullable type arrays) — the right semantics for the Preview Form tab. `src/components/schemaEditorModal.ts::getPreviewValue` switches to the new helper and normalizes `JSON.stringify(undefined)` → `'{}'` so the preview element always receives a valid JSON object. Defensive fallback in `_renderNumberInput` (`src/schema-editor/modes/form-mode.ts`): new `formatNumericForInput(data, schema)` renders `data === 0 && !('default' in schema)` as empty. Tests: new `src/schema-editor/bh010-preview-zero.test.ts` (10 cases), plus `bugfix-batch-10/11/12` reworked to match preview semantics (the old assertions were codifying the bug — nullable numerics expecting 0 instead of null, string props expecting `''` instead of dropped-as-undefined). E2E: `e2e/tests/c15-bh010-preview-form-numeric.spec.ts` proves an integer field with `minimum: 1900, maximum: 2100` now renders empty and produces no bogus range error on blur. |
...
| BH-005a | **fixed** (2026-04-22, c18-obs-search-docs, PR #41 merged cd4f0bf3) | Split from BH-005 (original report bundled case-sensitivity + fuzzy tolerance). On SQLite, wrapped column + pattern in `LOWER()` in `application_context/search_context.go::searchEntitiesLike` and `fts/sqlite.go::fuzzyFallback`. Postgres unchanged (already uses ILIKE). FTS5's `unicode61` tokenizer case-folds at index time, so the FTS path was already case-insensitive — this fix aligns the LIKE fallback + fuzzy fallback with the FTS behaviour. API tests: `server/api_tests/global_search_case_insensitive_test.go` (3 cases: FTS, LIKE fallback, FTS fuzzy). E2E: `e2e/tests/c18-bh005a-search-case-insensitive.spec.ts`. **Remaining scope filed as BH-005b** (typo tolerance + Unicode case-folding — needs design brainstorm). |
| BH-022 | **fixed** (2026-04-22, c18-obs-search-docs, PR #41 merged cd4f0bf3) | `server/routes_openapi.go` gains 11 new registrations: `POST /v1/mrql`, `POST /v1/mrql/validate`, `POST /v1/mrql/complete`, `GET/POST/PUT /v1/mrql/saved`, `POST /v1/mrql/saved/delete`, `POST /v1/mrql/saved/run`, `POST /v1/{note,group,resource}/editMeta` (via a new `registerMetaEditRoute(entity, tag)` helper), and `POST /v1/plugins/{pluginName}/display/render`. New drift-check unit test `server/openapi/drift_test.go` walks the live mux, diffs every `/v1/*` route against the registry, and fails if a new route is added without a corresponding registration (or an explicit `routesExcludedFromOpenAPI` entry with a reason). `/v1/plugins/` PathPrefix handler is in the exclusion list (dynamic plugin-specific API). Spec path count: **156 → 166** (drops one because `/v1/mrql/saved` has GET+POST+PUT on the same path entry). |
| BH-032 | **fixed** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a) | New `withSecurityHeaders` middleware in `server/share_server.go` wraps the ShareServer's router on both the `Handler()` (test) and `Start()` (production) paths. Sets `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer` (stops share-token Referer leaks to external fonts/images), strict `Content-Security-Policy` (default-src 'self' + unsafe-inline for Alpine + data:/blob: for previews + frame-ancestors 'none'), and `Strict-Transport-Security: max-age=15552000`. Error paths keep every header. Primary server ships the CSP-free subset via a parallel `withPrimarySecurityHeaders`; strict CSP on the primary broke Alpine/axe timing on admin pages and is tracked as a follow-up. `BuildPrimaryRouter(...)` extracted so the OpenAPI drift test can walk the raw router. API tests: `server/api_tests/share_server_security_headers_test.go` (share + primary). |
| BH-033 | **fixed** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a) | New `MahresourcesConfig.SharePublicURL` field, `-share-public-url` flag, `SHARE_PUBLIC_URL` env. When set, `note_template_context.go` trims the trailing `/` and uses it as the share URL base. When unset, shareBaseUrl stays empty and `shareUrlConfigured=false` flows to `templates/partials/noteShare.tpl`, which renders a warning pointing at the config key plus the relative `/s/<token>` path — the old bind-address fallback (which produced non-routable `http://127.0.0.1:<port>/...` URLs on any non-loopback deployment) is gone. CLAUDE.md config table gains `-share-port`, `-share-bind-address`, and `-share-public-url`. API tests: `server/api_tests/share_url_public_url_test.go` (unset → no bind-address URL, set → absolute URL, trailing-slash normalization). E2E: existing 19-note-sharing tests updated by passing `-share-public-url` in the ephemeral server harness. |
| BH-035 | **fixed** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a) | Nullable `Note.ShareCreatedAt *time.Time` column added via GORM auto-migration (existing rows stay NULL and render "(unknown)"; no back-fill). `ShareNote` now stamps both `share_token` and `share_created_at` in a single `Updates()` call; `UnshareNote` clears both the same way. New `GetSharedNotes()` and `BulkUnshareNotes(ids)` methods on `MahresourcesContext` + `NoteSharer` interface. New `/admin/shares` page (template `adminShares.tpl`, context provider `admin_shares_template_context.go`) — columns Name / Public URL / Created / Revoke. Bulk revoke via top-of-table button; per-row revoke via a hidden per-row form targeted by the HTML5 `form="..."` attribute (nested forms are invalid HTML). New API endpoint `POST /v1/admin/shares/bulk-revoke` (form-encoded `ids=<noteId>`, 303 redirect for browser forms, JSON for `Accept: application/json`). Admin menu gains a "Shares" entry. Registered in OpenAPI spec. API tests: `server/api_tests/admin_shares_test.go` (4 cases). E2E: `e2e/tests/c9-bh035-admin-shares-dashboard.spec.ts` (list / bulk revoke / single revoke / empty state). A11y config: `/admin/shares` added to STATIC_PAGES. |
| BH-038 | **fixed** (2026-04-22, c9-share-surface, PR #42 merged c8ac746a) | `server/template_handlers/template_context_providers/note_template_context.go::NoteListContextProvider` now maps each note through a shallow copy with `ShareToken` and `Blocks` nil'd out, then sets a new transient `Note.HasShare bool` (gorm:"-"). The copy protects the upstream GORM session from mutation. `partials/note.tpl`'s `{{ entity|json }}` serialization into Alpine `x-data` no longer carries the share token in the rendered HTML — plus the same fix flows through `/notes.json` by the same provider. API test: `server/api_tests/notes_list_no_share_token_leak_test.go` covers both HTML and JSON surfaces. |
| BH-037 | **fixed** (2026-04-22, c18-obs-search-docs, PR #41 merged cd4f0bf3) | Three-surface fix: (1) `models/resource_model.go` declares `ImageHash *ImageHash` as a 1:1 reverse relation (`foreignKey:ResourceId;references:ID`) so `GetResource`'s existing `Preload(clause.Associations)` auto-loads it. (2) `templates/displayResource.tpl` Technical Details section renders a `data-testid="perceptual-hash-row"` panel with `DHash: 0x...` and `AHash: 0x...`; when DHash is zero, shows an amber "uniform/solid-colour image (BH-018)" hint explaining the false-positive class. (3) New `ShowDhashZero` flag on `ResourceSearchQuery` + `models/database_scopes/resource_scope.go` scope; `application_context/admin_context.go`'s `SimilarityInfo.DhashZeroCount` populated in `GetExpensiveStats` (matches both migrated `d_hash_int = 0` and legacy `d_hash = '0000000000000000'`); `templates/adminOverview.tpl` renders a `data-testid="admin-dhash-zero-drilldown"` link to `/resources?ShowDhashZero=1`, gated by Alpine's `x-if="dhashZeroCount > 0"` so empty deployments don't see a dead link. Tests: `server/api_tests/resource_image_hash_preload_test.go`, `server/api_tests/resource_dhash_zero_filter_test.go`, `server/api_tests/admin_dhash_zero_stats_test.go`. E2E: `e2e/tests/c18-bh037-perceptual-hash-visible.spec.ts`. |
| BH-014 | **fixed** (2026-04-22, c16-group-ux, PR #43 merged 38a3d012) | New `src/components/confirmGroupDelete.js` Alpine component replaces the generic `confirmAction` binding on the bulk-delete form in `templates/partials/bulkEditorGroup.tpl`. On submit: fetches each selected group via `GET /v1/group?id=<id>` in parallel, aggregates `OwnGroups + OwnNotes + OwnResources` counts (capped at the API's 50-per-association page limit — larger sums render as "50+"), and calls `window.confirm("Delete N groups? This will orphan X child groups and M notes/resources (they'll move to top level).")`. Leaf-only selections see a simpler "Delete N groups?" with no orphan language. Fetch failures fall back to a generic confirm. Shift-submit bypass preserved from `confirmAction` as a power-user escape hatch. E2E: `e2e/tests/c16-bh014-group-delete-orphan-warning.spec.ts` (2 cases: parent-with-children shows counts, leaf-only shows no orphan language). |
