...
Deferred plugin downloads stay under `db:write` for the same reason immediate `mah.download.submit` does: one URL is fetched into the library by the host; only the start time changes, bounded at thirty days so this is not recurring unattended plugin code. `{start_at=<unix seconds>}` and `{delay="2h"}` are mutually exclusive; both return `{scheduled=true, scheduled_id=<row id>, start_at=...}` rather than `id`, because no queue job exists yet. The row is durable and **not** resident in the in-memory queue, whose 100-job cap never evicts pending jobs. It stores the plugin name and payload, including headers, so a restart does not turn the fire into an unrestricted host fetch.
