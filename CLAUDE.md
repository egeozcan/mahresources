...
- **A variant naming `AUDIO="group"` carries no audio of its own.** Its tracks live in an `EXT-X-MEDIA` rendition, so downloading only the variant produces a silent video that plays perfectly — the worst failure available, since nothing looks wrong until someone listens. The rendition is fetched as a second media playlist into its own subdirectory (both number their segments from zero) and muxed as a second input with explicit `-map`s. A rendition with no `URI` is already multiplexed into the video segments and needs nothing.
- **`EXT-X-BYTERANGE` with no `@offset` means "the byte after the previous sub-range of this resource"**, and the parser reports that as 0 — so an untracked playlist of sub-ranges fetches the *first* n bytes every time and muxes the opening fragment repeatedly. A running end is kept per URL. A server that ignores `Range` and answers 200 with the whole object is sliced locally rather than stored once per sub-range.
