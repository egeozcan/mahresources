...
- **`trim` is refused inside `mah.db.transaction`**, because it runs ffmpeg over the whole clip and writes a version, and inside a transaction that is the database's write lock held for the length of a transcode.
