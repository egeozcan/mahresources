...
The Job Service is the durable control plane for user- and operator-facing background work. It accepts a Job before execution, stores lifecycle, ownership, replay input, claims, capacity, events, outputs, and lineage, and fences executor writes with an execution token. Downloads, exports, imports, Resource Reduction computation, maintenance, plugin actions, plugin command runs and imports, and schedules use this shared plane with their specialized executors. `job-summary-export@1` is a durable owner-visible Kind whose filtered queries use the same visibility predicate as interactive summary. Its sealed filter is decoded with plain JSON, so a worker from an older release silently drops a filter field it does not know (the inbound relationships) and fails on a token it does not know (`state=partial`); dimensions added after v1 are therefore refused for exports in `unsealableSummaryFilterDimension` rather than sealed, and the interactive summary keeps them. The external Job Center API and UI are enabled; startup migration readiness checks and the registered Kind inventory guard the cutover.
...
- **Acceptance and outcomes:** a canonical download Job is accepted before the transfer starts. The queue's `HistoryRecorder` still writes the legacy terminal history projection. A failure in that legacy write does not replace the canonical Job outcome.
- **Retry identity:** Retry of canonical work creates a new Job with a `retry-of` link and leaves the source Job's outcome unchanged. The legacy handle follows the current Retry leaf, while a canonical UUID always names one Job. An old history entry without a canonical Job reference may use the legacy resubmission path.
- **Compatibility reads and controls:** existing download, job, history, and SSE routes continue to expose their established legacy representations. A compatibility handle resolves to the current Job for get/list/events/control. Canonical SSE uses `version=2` and its own cursor/identity format. Keep these projections aligned with the Job Service when changing legacy routes.
...
**Job progress metrics and graphs.** A progress tick is a snapshot on the Job row, never a Job Event, and it never moves the Job's version. Beside the primary measure it carries up to 8 metrics (`models/jobmetrics`, a leaf under `models/` so that `plugin_system` and `plugin_commands` can validate a metric without importing `jobs`), 3 of them graphable. `jobs.advanceSeries` keeps a bounded progress series on the row: one point per interval, compacted by merging pairs and doubling the interval at 120 points, first point kept. Speed and ETA are derived in Go (`Snapshot.LiveRate`, `ExpectedFinish`) so the API, the SSE frame and the `/jobs` template agree. Live delivery is a `job-progress` frame on the canonical stream after `job-caught-up`, read through `Service.LiveProgress` under the same visibility predicate as events. Like `job-caught-up` it **has no SSE id**: giving it one would move the durable delivery cursor. Its timestamps are not a cursor either — each process writes its own clock — so each poll reads a 30 s window and dedupes per connection by the snapshot last sent. Plugins report through the table form of `mah.job_progress` (a report the 200 ms throttle held back is flushed at settlement) and command runs through `::mah-progress` stdout lines, which are stripped from the output tail and redacted with the run's secrets. The command mirror writes through `Service.UpdateProgress` with a fresh handle, not through the claimed `jobs.Execution`: `claimPluginCommandJob` claims inside a transaction, and an Execution carries the handle it was claimed with.
...
- **`Deps.CheckURL` is required, and a nil one refuses everything.** It is asked for the *submitted* URL too, not only what a playlist names: a plugin's URL was checked when it was submitted, but a retry replays a stored payload, possibly in another process and possibly after an operator narrowed that plugin's network list. The client's decoration carries layers (b) and (c) — the per-redirect re-check and the dial-time deny — but *not* layer (a), the allowlist, whose comment in `ApplyEgressPolicy` says outright that the caller applies it before the request starts. Every other caller in the tree holds exactly one URL and checks it once. A playlist names more, and none of them passed through a caller, so without this a plugin confined to `a.example` reaches any public host by being served a playlist that says to. It is asked before **every** request — variant playlist, key, init segment, each segment — and `TestEveryRequestedURLIsCheckedAgainstTheAllowlist` pins that with two servers that differ only by port.
- **Callers pass the URL the playlist was *served from*, not the one they asked for.** A redirect from `/watch` to `/cdn/abc/index.m3u8` moves the base by a whole directory, and every relative segment reference resolves against it; `fetchText` likewise returns the final URL of a nested playlist.
- **A variant naming `AUDIO="group"` carries no audio of its own.** Its tracks live in an `EXT-X-MEDIA` rendition, so downloading only the variant produces a silent video that plays perfectly — the worst failure available, since nothing looks wrong until someone listens. The rendition is fetched as a second media playlist into its own subdirectory (both number their segments from zero) and muxed as a second input with explicit `-map`s. It shares the download's byte and segment budgets rather than getting its own, or a playlist would spend twice the configured cap by splitting its streams. A rendition with no `URI` is already multiplexed into the video segments and needs nothing.
- **`EXT-X-BYTERANGE` with no `@offset` means "the byte after the previous sub-range of this resource"**, and the parser reports that as 0 — so an untracked playlist of sub-ranges fetches the *first* n bytes every time and muxes the opening fragment repeatedly. A running end is kept per URL, and applied only where the **raw text** carried no `@`: the parser represents an omitted offset and an explicit `@0` identically, they mean opposite things, and guessing either way corrupts the other, so `explicitByteRangeOffsets` scans the playlist text to tell them apart. A server that ignores `Range` and answers 200 with the whole object is sliced locally rather than stored once per sub-range.
- **The sniff's read error is checked.** `io.ReadFull` returning anything but `EOF`/`ErrUnexpectedEOF` is the transfer failing, and discarding it stored whatever had arrived as a complete resource — a connection dropped ten bytes in became a ten-byte file reported as a successful download.
...
- **Neither `pl.Key` nor `pl.Map` is read; the parse is driven entirely by the segments.** The parser publishes the first of each tag twice — once on the playlist "for convenient playlist generation" and once on the segment it precedes — and `pl.Key` is set from the first `EXT-X-KEY` *wherever* it appears, so a stream that starts in the clear and turns encrypted seeded as encrypted and decrypted its clear segments. Reading both sources also consumed the raw-text explicit-offset marker twice, refusing ordinary ranged fMP4. Every `KEY` and `MAP` tag attaches to exactly one segment, in text order, so one source is both correct and simpler. Whether the initialization section is encrypted is the one thing the parse cannot answer — every tag above the first segment attaches to it, and when two keys sit there only the last survives — so `headerKeys` reads the raw text for the key in effect at the map and at the first segment. Both wrong answers produce a file that decodes to nothing without naming a cause, and a stream that protects its map with a *different* key is refused rather than assembled with the wrong one: that key is not in the parse at all.
- **A key stays in effect until another `EXT-X-KEY` replaces it**, which the parser does not express: it attaches a key only to the segment the tag preceded. Falling back to the playlist-level key for every untagged segment decrypted a rotated stream entirely with its first key — and read a mid-stream `METHOD=NONE` as "still encrypted", since that arrives as a tag whose parsed key is nil, indistinguishable from no tag unless the tag itself is tracked. The initialization section's own key is recorded and written **before** the `EXT-X-MAP` line, because ffmpeg decrypts the map with whichever key it has seen by the time it reads it.
- **Bytes are charged where they cross the network, not where they are kept.** A server that ignores `Range` and sends the whole object has its skipped prefix charged to the budget too, or a playlist of one-byte ranges at gigabyte offsets pulls down gigabytes under a one-byte cap. The key ceiling bounds the *read* rather than checking the total afterwards, so a hostile key endpoint cannot spend the whole download budget before being refused.
- **A ranged fetch must deliver exactly what it promised.** The range reader is bounded rather than checked, so a short response left the missing bytes simply absent and the mux failed opaquely. A `206` is refused unless its `Content-Range` parses and starts where the request asked; an `EXT-X-MAP` byte range with no explicit offset is refused rather than guessed, because the running offset that places media sub-ranges cannot see how the two tags interleave.
- **Every request carries the deployment's idle bound** (`Deps.IdleTimeout`), not only the one the caller opened. The overall deadline is no substitute: a server that sends headers and then stalls holds a worker for the whole of it, and a playlist has hundreds of chances to do that. The guard closes the body before waiting for its pending read, so nothing writes into the caller's buffer after the read has returned.
...
- **The queue resolves the ffmpeg path per download, not at construction.** Startup auto-detects ffmpeg on `PATH` *after* the context and its download manager exist, so a captured string stayed empty in the most ordinary deployment there is — ffmpeg installed, no `-ffmpeg-path` — and every queued HLS download failed saying ffmpeg was unavailable while the boot log said it had been found. The synchronous path reads it live and worked, so the two disagreed. `ManagerConfig.FfmpegPath` is a `func() string` for that reason.
- **The segment cap bounds *parsing*, not just the result.** The parser materializes every segment it is handed, so a playlist well inside the byte limit — a million one-character entries is a few megabytes — allocates hundreds of megabytes before a check on the result could refuse it. `parse` counts `#EXTINF:` in the text first; `readMedia` still enforces the same limit on what it accepts.
...
Every fetch the host makes on a user's behalf went out as `Go-http-client/1.1`, which at least one supported platform's media endpoint answers with 403. `hostfetch/` is a leaf package, in the shape `hls/` established, holding the browser-like default, the intake validation and one `http.RoundTripper` decorator.

- **One choke point, not one edit per request site.** The clients are already built per call and already carry the caller's egress policy, so the decoration composes with `ApplyEgressPolicy` and covers the initial GET, every HLS playlist, key, rendition and segment, and every redirect hop — with one implementation rather than three. `mah.http` is deliberately untouched: that is a plugin fetching under its own honest agent, not the host downloading for a person.
- **The User-Agent goes everywhere; a caller's own headers go to the submitted URL's host only.** A `User-Agent` *among* those headers is therefore lifted out and becomes the download's agent on every host: an endpoint that refuses one agent refuses it on its CDN too, so binding it to the submitted host would fix the playlist and leave every segment failing — and it names the fetcher rather than the user, which is the same reason the deployment's own is safe anywhere. An empty value anywhere means the default, never an empty header line. A reviewer put the counter — a caller could encode a secret in it, and it then reaches a host the playlist chose — and that is the accepted cost: the User-Agent is the one header whose purpose is to be announced to every server a transfer touches, the deployment's own already is, and the header map bound to the submitted origin is where a value that must not travel belongs.
- **The bind is to the *origin*, not the host.** Scheme and effective port are both part of it: a redirect from `https` to the same name over plain `http` is a downgrade, and matching on the host alone put a `Cookie` on the wire in clear.
- **Map headers are `Set`, not filled-if-empty.** net/http writes its own `Referer` on every redirect, which a fill-if-empty rule mistook for a header the calling code owned — so a caller's `Referer`, the single most likely reason to use this at all, survived the first request and was replaced on every hop after it. Nothing else collides: `hls` sets only `Range`, and `Range` is refused at intake. The *User-Agent* keeps the fill-if-empty rule, because there the calling code's own value is the more specific one.
- **`SubmitForPlugin` copies the map it validated.** `SubmitMultiple` gives every job a shallow copy of one creator, so all of them would otherwise share a map the submitter still holds — one that can change after the validation that approved it, while a worker iterates it and the history row is serialized from it. The agent identifies the fetcher and says nothing about the person who asked, so no host is unsafe. A playlist, by contrast, names further URLs and the host policy permits every public host, so replaying a `Cookie` or an `Authorization` onto whatever a playlist says is a credential handed to a server the *content* chose. The same rule makes the decoration redirect-safe for free: net/http strips `Cookie` and `Authorization` across a domain change, and a decorator re-adding them per attempt would silently undo that. The comparison is **host:port**, not hostname — two httptest servers differ only by port, so a hostname rule passes a cross-host test while sending the header anyway. A header the calling code already set wins, because `hls` sets its own `Range`.
- **Every URL of a newline batch is decorated separately.** `AddRemoteResource` fetches each line, and each line is a URL the caller submitted with these headers, so all of them receive them — but each binds to its *own* host, which is what keeps them off the hosts a playlist beneath any of them goes on to name.
- **Refused at intake, not at request time.** Connection-level and framing headers (`Host`, `Content-Length`, `Connection`, `Keep-Alive`, `Transfer-Encoding`, `Upgrade`, `TE`, `Trailer`, and the whole `Proxy-*` family by prefix rather than by the three names anyone thinks to list) are net/http's to own, and `Range` is `hls`'s — a caller-supplied one misassembles the media silently and breaks the playlist sniff. `SubmitForPlugin` is the single door every queue submission passes through (the API handler, the plugin submitter, `SubmitMultiple`, a retry), and `AddRemoteResource` checks before it fetches anything. By the time a worker builds a request the submitter is gone and the only surface left is a failed history row.
- **The headers live on `ResourceFromRemoteCreator`,** which is what persists them on the download history payload and replays them on retry — a download that only worked with a `Referer` must still work when it is retried on another worker months later. It also means a `Cookie` is stored in the database until the row is swept; the payload is never rendered to a page. They are `schema:"-"`: gorilla/schema decodes no map, so this is a JSON-body option and the create-resource form carries none.
- **The plugin option is one helper read by both surfaces** (`applyRemoteHeaders`), so an author writes one shape of options whether they call `mah.db.create_resource_from_url` or `mah.download.submit`. No new capability: a header widens no egress, the transfer is policed exactly as before, and `mah.http` already sends arbitrary headers to allowlisted hosts.
- **The setting is runtime-editable** (`remote_user_agent`), because a 403 from one platform is a thing to fix without a restart. Empty anywhere means "not configured" and selects the default; it never means "send no User-Agent", which is the value that produced the 403.

### Plugin downloads and media

Two plugin surfaces built on the host work above: `mah.download.submit`, and `mah.media`.

**`mah.download.submit(url, opts)` is gated on `CapDBWrite`, deliberately not on a name of its own.** The power is `create_resource_from_url`'s — fetch a URL of the plugin's choosing into the library, which that capability's label already states — differing only in whether the caller waits. It is not `CapJobs` either: `jobs` means a plugin's *own code* runs in the background, and none of it runs here; the host downloads. What it buys is the two things a synchronous fetch cannot give, since `create_resource_from_url` holds the plugin's VM lock for the length of the transfer and, inside an async job, dies at `MaxAsyncJobDuration`. `internal/arch/plugin_capability_gate_test.go` pins the gate, so folding it under a different capability fails the build rather than silently widening what a `jobs` or `db:read` plugin can do.

- **The transfer runs under the *plugin's* declared network list, never the host policy**, which allows every public host. `DownloadJob.pluginName` selects it; `createHTTPClientFor` refuses when it cannot be resolved rather than falling back — an unresolvable plugin is one that was disabled, renamed, or whose resolver was never wired, and every one of those is a "no". The resolver is set through `SetPolicyResolver` *after* construction, in the `SetKVStore` shape, because the manager is built while the context is still assembling itself and a closure passed through `ManagerConfig` would capture a nil plugin manager.
- **The origin is persisted on the history row** (`DownloadHistoryEntry.PluginName`) and carried on the upsert's update arm. A retry replays the stored payload on a fresh worker, possibly in a process that never saw the job; one that forgot the origin would silently become a host fetch. Same doctrine as `validateDownloadScope`: a payload records what was once asked for, not a standing permission.
- **Layer (a) is checked at the call site too**, as `create_resource_from_url` checks it before creating anything. It is not the protection — the transfer is policed again when it runs — but the difference between telling the plugin now and letting it find out minutes later in a history row.
- **The targets are validated against the acting principal's scope**, mirroring `validateDownloadScope`, which cannot be reused because it takes a principal off a request and there is none here. Without it a plugin's Lua running on a confined user's own write could submit a download owned by a group that user cannot see, and the worker — unscoped by design, binding only attribution — would create it. The principal is resolved from the actor id through `principalForPluginActor`, the same resolver `mah.db` binds through, so a deleted or disabled account is deny-all rather than unscoped.
- Refused inside `mah.db.transaction`: a queued download outlives the transaction waiting for it. The returned status comes from `job.Snapshot()`, not the live job: `Submit` starts the worker before it returns.

### Plugin download pacing and deferred downloads

A plugin can narrow its own `mah.download.submit` traffic with a manifest-level `download_limits` list. It is deliberately **not** a capability and deliberately not part of consent: a throttle grants no new power, affects no other principal's jobs, and removing it only returns the plugin to the download power `db:write` already granted. `host` reuses the `network` grammar and `NetworkRule.Matches`, `api_version` is required once the key is present, and the first matching rule wins so an author can put an exact exception before a wildcard. Wrong shapes are refused by name at parse time; a typo in `concurrency` must not silently mean unlimited.

The queue gate is job-level and process-local. It sits in `download_queue`, takes no dependency on `plugin_system`, and is wired through `SetThrottleResolver` in the same late-binding shape as the plugin egress resolver. It is acquired **before** the global semaphore — including backoff and min-interval waits — because a domain that is waiting politely must not occupy one of the deployment-wide worker slots. The gate is inert at its zero value so struct-literal test managers and deployments with no declarations do not change. Its mutex is independent of `dm.mu` and `j.mu`, and it is never held while waiting, notifying, recording history or emitting job events. `min_interval` reserves its assigned start time under the lock (`lastStart = max(now, lastStart+interval)`) rather than checking and then sleeping, or two jobs can read the same old value and start together. A `429` or `503` charges backoff against the **submitted** URL's host, the key future jobs will match, not a redirected final host. `Retry-After` is honored but clamped to the declared backoff ceiling. HLS segment requests are out of scope for this gate — one HLS job may still use `-hls-concurrency`, and `hls` handles segment retries itself — and generic export/import/action jobs never enter `processJob` because they have no remote URL to key.

Deferred plugin downloads stay under `db:write` for the same reason immediate `mah.download.submit` does: one URL is fetched into the library by the host; only the start time changes, so this is not recurring unattended plugin code. `{start_at=<unix seconds>}` and `{delay="2h"}` are mutually exclusive. A relative `delay` is capped at thirty days. An absolute `start_at` must be in the future and has no upper bound. Both return `{scheduled=true, scheduled_id=<row id>, start_at=...}` rather than `id`, because no queue job exists yet. The row is durable and **not** resident in the in-memory queue, whose 100-job cap never evicts pending jobs. It stores the plugin name and payload, including headers, so a restart does not turn the fire into an unrestricted host fetch.

A scheduled download insert is principal-bound before `Create`, because the stamp callback overwrites `CreatedByUserId`; setting the field on the struct would be replaced by the worker/default actor. The plugin scheduler's existing ticker runs the due-row sweep after the `pm == nil` early return — without a plugin manager, no plugin policy can be resolved and every row must refuse. Firing claims with one conditional `UPDATE`, re-validates the stored actor's role and scope, checks the plugin is still available, and reserves `status=submitted` **before** calling the queue so a stale claim cannot double-submit. The cost is fail-closed: a crash between reserve and submit, or a DB failure after submit, can strand a submitted row without a job id and needs operational repair, but it will not fork the download. Owner deletion NULLs `created_by_user_id` through `stampedModels()`, and ownerless pending rows are never claimed rather than falling back to root.

**`mah.media` is `CapMedia`, separate from `CapImage`.** `mah.image` transforms bytes the plugin already holds and touches nothing else; `mah.media` reads video and audio *out of the user's library* and spends an ffmpeg process doing it, so folding them together would silently widen every plugin already consented to image transforms — the widening `CompareGrants` exists to report.

- **The seam is deliberately not a new wiring point.** `mediaProcessorFor` reaches through `querierFor` — the function the whole `mah.db` surface goes through — and type-asserts what `BindInvocation` returned to `MediaProcessor`. Every call is therefore already bound to the acting principal, so a group-limited caller cannot probe or cut a resource outside its subtree: a property of the handle rather than of a check somebody has to remember. A seam of its own would have been a second path to the same files with none of that. A compile-time assertion on `*pluginDBAdapter` keeps the assertion from quietly starting to answer nil.
- **A plugin names a resource id and a number, never a path and never an ffmpeg argument.** `probe` returns the ffprobe document whole (`format` plus `streams`), because which field matters varies by caller and a curated list is one somebody keeps extending. `extract_frame` returns the `data:image/jpeg` URI `mah.image` already speaks, so the two compose without a conversion step.
- **`ExtractVideoFrame` is a sibling of `createThumbFromVideoFileAtTime`, not a caller of it.** That one exists to make a thumbnail, so it rounds the timestamp to a whole second and scales to a fixed 640px — right for a thumbnail, wrong for an API whose caller asked for a particular moment. It shares the video thumbnail lock and timeout, so plugins cannot start more concurrent ffmpeg processes than the deployment already allows for the same work, and it falls back to documented defaults when those are zero: a context built from a bare config (the CLI, every test) would otherwise refuse every frame with a message about a timeout nobody configured.
- **Every ffmpeg-family call takes the deployment's video gate**, `probe` and `extract_frame` through `runVideoTool` and `trim` through `trimVideoGated`. Trim is gated *around* `TrimVideo` rather than inside it, because the bound belongs to the plugin surface — a person clicking the trim button does one at a time, a plugin loop does not — and moving it inside would change the HTTP endpoint too. It cannot deadlock: `TrimVideo` takes a different lock, and nothing beneath it takes this one, since thumbnails are generated lazily on request. It carries no run timeout, because a trim is a transcode and the thirty seconds that bounds a frame would refuse every clip over a few minutes.
- **Waiting for the gate is bounded by the caller's own deadline** (`videoLockWait`). The lock's wait takes no context, so a cancelled request or an expired async job would otherwise sit in it for the full configured minute doing nothing anybody is waiting for — a minute a schedule handler spends outside the budget its claim TTL is derived from.
- **The temp-file copy happens inside the gate**, under its deadline. On a filesystem with no local path, probing copies the whole file; doing that before taking a slot meant an unbounded number of concurrent copies, each able to outlive the caller's budget, and afero readers take no context so `cancelableReader` is where the question gets asked.
- **`runVideoTool` takes the gate first and derives its deadline second**, so a request that spent half its lock wait still gets a full execution budget — a deadline started before the lock is spent by the queue in front of the call, and one that waited the full lock timeout would hand ffmpeg a context already cancelled. It builds the command on that derived context rather than the caller's. That second half is the one that is easy to get wrong: `RunWithLockTimeout`'s run timeout bounds only how long *it* waits — it returns while the function keeps running — so a command built on the caller's context leaves a wedged ffmpeg alive with nothing holding a reference to it. `probe` had no gate at all at first, which made the cheapest call in the API the one with no bound on concurrent processes.
- **The write surfaces check the acting principal's *role*, not only its scope** (`requireWriteRole`, beside `requireTaxonomyRole` and `requireEditorRole`). `mah.media.trim` and `mah.download.submit` both write, and a plugin reaches them from a page a **guest** is entitled to read — a shortcode or an injection runs as whoever loaded the page, so the URL rule that refuses a guest every mutating endpoint is nowhere in the path. Scope cannot answer it either: the guest's own subtree is exactly where the write would land. This is the general form of the hazard CLAUDE.md already records for hooks, applied to two new doors.
- **All three are refused inside `mah.db.transaction`**, not only `trim`. Each waits up to a minute for a video slot, may copy the file, and then runs a process; inside a transaction that is the database's write lock held for the whole of it, and a read that takes a minute is as bad as a write that does.
- **`trim` takes an `into` of `"version"` or `"resource"`.** The default replaces the resource's content, as the trim button does; `"resource"` files the clip separately and leaves the source alone, which is what a plugin pulling five clips out of a recording needs. `trimVideoBytes` is extracted so both share one definition of what the clip is, and `parseTrimRange` so they cannot drift on what a valid range is. An unrecognised `into` is refused rather than defaulted: a typo that quietly replaced a two-hour recording with a ten-second clip is not something an author finds out about in time.
- **The whole HLS assembly carries one deadline** (`Options.OverallTimeout`, the deployment's `-remote-overall-timeout`). `http.Client.Timeout` applies per *request*, so a five-thousand-segment stream whose server stalls each request just under it is bounded by nothing.
- **A `206` is only trusted if its `Content-Range` starts where the request asked**, and its body is length-bounded either way: a 206 that runs past the range it promised is the same corruption as a 200 nobody sliced.
- **ffmpeg and ffprobe get a real file, never a pipe.** Both need to seek, and an MP4 whose `moov` atom sits at the end cannot be read from stdin at all; `localOrTempPath` hands back the file itself on a local filesystem and copies to a temp file on any other, which costs a copy and always works.
