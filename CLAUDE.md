...
Group-limited principals are also denied every **plugin-code endpoint** (`/v1/plugins/...`, `/plugins/...` — the JSON API catch-all, block/display render, and plugin pages). The deny is fail-closed in `withAuthorization`; unscoped roles (admin/editor/unscoped user) are unaffected. Scope-aware (tree-based) plugin access is a planned follow-up.

**The deny matches URL paths, and hooks are not one.** They fire from ordinary scoped CRUD that a confined user is entitled to perform, so a plugin's Lua runs on a confined user's own write whatever that deny says — which is why `mah.db` is scoped to the *acting principal* rather than relying on the deny to keep confined callers out. `BindInvocation` resolves the actor id to the stored account (`principalForPluginActor`) and binds its real scope group; an actor whose account cannot be read — deleted or disabled since an async job or drained HTTP callback captured its id — is bound **deny-all for subtree-scoped data**, never unscoped. What binds is *scope*: role feeds attribution and decides whether a scope is required, but **nothing below `server/` is role-checked**. Role capability is decided entirely in `server/authz_policy.go`'s `principalSatisfies` — and note that the taxonomy gate there is its `default: return false` arm after the `p.IsAdmin()` short-circuit, not a call to `CanManageTaxonomy`, which has no production call site at all. (`CanWrite` is also read at `server/auth_handlers.go:221` to build the `/v1/auth/me` payload.) So a hook can perform an admin-only taxonomy write whoever triggered it. What stays reachable to a confined or deny-all principal is anything `scopeColumn` does not map: tags, categories, note types and **relation types**, none of which carries an owner. Relation *edges* are partly out of that set. Subtree containment for an edge is a property of two columns rather than one, so `EditRelation`/`DeleteRelationship` check both endpoints explicitly via `relationInScope`. Guarding the direct writes alone was worth little, because a relation type and a category both cascade to `DELETE FROM group_relations` database-wide, so there are two further layers: `refuseGlobalCascadeWhenScoped` rejects `EditRelationType`, `DeleteRelationshipType` and `DeleteCategory` early (before `before_category_delete` fires), and a GORM delete callback (`globalCascadeDeleteCallback`) refuses an ORM delete against `categories` or `group_relation_types` for a scoped principal. That covers the generic `CRUDWriter.Delete` that `CategoryCRUD()` hands out — constructed and passed to `server/routes.go`, though only its `ListHandler` is routed today, so it is a latent bypass rather than a live one. It does not cover a `db.Table("categories").Delete(&other)` that overrides the model's schema, nor association deletes under `SkipDefaultTransaction`; neither is used anywhere. **Three paths remain open and are known, not closed:** changing a group's own category deletes every incident edge that no longer matches, including one whose far endpoint is outside the subtree; group merge's final `DELETE FROM group_relations WHERE to_group_id = from_group_id` sweep has no subtree predicate, so a legacy self-edge anywhere is fair game; and `AddRelationType` writes `BackRelationId` onto an *existing* reverse type it finds, so "create touches no existing row" is not true (it cascades to no edge, but it is a write). The first two could be closed with subtree predicates on those specific statements; only the general case — a confined caller performing any admin-only taxonomy write — needs role capability below `server/`, which does not exist. `mah.db.mrql_query` is the one data path that bypasses `BindInvocation` (it has its own executor), so it carries the actor on `MRQLExecOptions` and binds through `WithMRQLPrincipal`, resolves SCOPE via `ResolveMRQLScope` and clamps with `effectiveMRQLRequestedScope` — a plugin asking for `scope = "global"` still gets its caller's subtree. Only a `uint` crosses into `plugin_system`; `internal/arch/plugin_auth_import_test.go` keeps it that way.
