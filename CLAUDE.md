...
- **`overlap = "allow"` buys queueing, not parallelism** — two runs of one plugin still serialize on its VM lock. It advances and releases at dispatch instead of at completion, so an overrunning run does not cause the next to be skipped.
