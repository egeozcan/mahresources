...
- **Auth-on accepted NULLs:** plugin `mah.db.*` writes (the plugin `EntityWriter` is a process-global bound to the singleton, with no per-execution principal — a documented v1 limitation), startup-time seeds/raw-SQL (default ResourceCategory, bootstrap seeds), and truly context-less background/worker creates (e.g. the hash/thumbnail workers, which have no submitter). Everything else (all 15 request-scoped/converted create paths + the two live raw-SQL paths + import + background remote downloads) is attributed per-user.
