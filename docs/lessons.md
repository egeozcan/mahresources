...
## Jobs drawer redesign — 2026-09-26

- The rule above (pi loop on every change) was broken again: a visual redesign with green unit and E2E tests was reported done without a single review round, and the first round found two P2s. A change that "only restyles" still moves markup that accessibility depends on; review it like any other.
- A test that fails on the branch and passes on master is not a flake until a baseline says so. Alternate runs of master and the branch (here 0/8 against 4/12) before calling it load, and instrument the failing path when it is real. That is how a master-side race was found: a refresh triggered by another job's event applied a failure silently, so its own late event announced nothing.
- Announce against what the reader has been told, not against the rows on screen. Rows are dropped, rolled back and re-read by three list reads, detail reads and stream messages, and every per-path diff opened another gap. A per-job ledger fed by one function, with reads deferring to live lifecycle events at the reconnect boundary, closed them; patching orderings one at a time did not converge.
- Check what the live stream actually sends before designing around it. The canonical SSE stream carries event-only messages (type and version, no snapshot), which made the snapshot paths test-only and made event types the only liveness signal.
