...
- [x] Read the approved design (§3, §5, §7, §9), ADRs 0006/0007, the plan's Task 3/4/5 contracts, `CLAUDE.md`, and the current `jobs/` and `models/` code.
- [x] Reproduce each finding as a red behavior test at the confirmed public seam before changing production code, including the two interleavings that need a second connection and one that only PostgreSQL can express.
...
|---|---|---|
| Artifact cleanup permanently starves later expired artifacts | `the artifact behind the protected one is expired (removed at <nil>), want the bytes gone and recorded` for the older quarantined artifact, and `the artifact behind the unaccountable one is expired (removed at <nil>), …` for the older artifact of a Kind no adapter can run — both with a one-item batch | The batch is selected without the Jobs an unresolved claim protects, and a candidate this pass could not establish as gone is given a later instant to be asked about again (`job_outputs.next_cleanup_at`): the scan orders by `COALESCE(next_cleanup_at, expires_at)` and skips what is not due (`TestRetentionArtifactCleanupReachesTheArtifactsBehindOneItCannotRemove`, two sub-cases) |
| Artifact deletion is not fenced against a newly acquired claim | `a claim committed while the artifact's bytes were still being deleted: <nil>` — through `Sweep` and `Claim` on two connections, on SQLite and again on PostgreSQL | The deletion runs inside one transaction that takes the Job's own row first and re-reads the protection under it, so a claim that committed while the pass waited is visible before anything is asked of the Kind, and a claim that arrives later waits for the deletion (`TestRetentionArtifactCleanupIsFencedAgainstANewClaim`, `TestRetentionArtifactCleanupIsFencedAgainstANewClaimPG`) |
| `Accept` codec failures expose secret replay input | `the sanitize failure contains the exact secret "url-query-secret-8f1c"` (and the Cookie, Authorization and plugin values), the same for the encode failure | `sealReplay` returns the classification and the Kind/version for `Sanitize` and `Encode` failures and never the codec's own text, which quotes the input the caller just submitted (`TestReplayAcceptanceCodecFailuresNeverQuoteTheSubmittedInput`, both hooks) |
| Public lifecycle paths can leave running Jobs outside claim recovery | `transition scheduled -> running = <nil>, want ErrRunningRequiresClaim` (queued and paused alike, and running → running), plus ~25 lifecycle-walking test sites failing with the same message; the release tests were compile-red on the old signature | `Transition` refuses `To: running` (`ErrRunningRequiresClaim`) and `legalTargets` drops running from every target set, so a Job is running only under the claim that admitted it; `ReleaseClaim` takes a `ReleaseRequest{ExecutionRef, Reason, To}` and writes the adapter-decided state together with the release, refusing a release of a running Job that names no state (`ErrReleaseNeedsState`) and keeping a release under a foreign token a no-op (`TestJobRunningAdmissionIsTheClaimProtocol`, `TestQuiescedReleaseMovesTheJobAndClaimsItBack`, `TestReleaseClaimRefusesToStrandARunningJob`) |

## Verification

- `go test --tags 'json1 fts5' ./jobs -count=1` — passed; `-race`, `-count=1` — passed.
- `go test --tags 'json1 fts5 postgres' ./jobs -count=1` — passed, including the new PostgreSQL fence regression.
- `go test --tags 'json1 fts5' ./application_context -count=1` — passed (61s); with `postgres` — passed (79s).
- `go test --tags 'json1 fts5' ./... -count=1` — every package passed.
- Mutation checks (each defect is caught by the test written for it, and the source was restored): dropping the claim exclusion from the artifact selection fails the protected sub-case; dropping the cleanup deferral fails the unaccountable sub-case; dropping `FOR UPDATE` from the PostgreSQL half of the fence fails the PG regression; neutering the SQLite guarded write fails the untagged one; making the release fall back to leaving the Job running fails `TestReleaseClaimRefusesToStrandARunningJob`.
- `go vet --tags 'json1 fts5' ./...` and `--tags 'json1 fts5 postgres' ./jobs` — clean. `gofmt -l` on every changed file — clean (`models/query_models/filter_decode.go` is gofmt-dirty on `main` and untouched here). `git diff --check` — clean.
...
- **Fairness is two mechanisms, one per class of candidate, and both are durable.** A protected candidate is excluded from the batch itself — the exclusion and the fence are the same predicate, in the shape `expiredJobPredicate` and `sweepProtected` already have — and a candidate no adapter could account for is deferred on its own row. The alternative shapes were a cursor in `Service` (rejected: `ReconcileExpired`'s retry schedule is durable for the same reason — two processes would each hold their own position) and a scan window inside one pass (rejected: it makes the pass's reach depend on how many candidates it cannot act on, which is the defect).
- **Deferral is a fixed interval, not the claim's widening schedule.** The reconciliation schedule widens because a broken reconciler otherwise holds the batch; here the batch is held by ordering alone, so the question a widening answers is retry *rate*, not fairness. `DefaultArtifactCleanupRetry` (15m) is deliberately shorter than `MaxReconcileRetry`: an artifact's deadline has its own clock and bytes promised gone an hour ago should not wait long for the pass that can remove them.
- **The fence is the Job's own row, taken exactly as `lockPruneTarget` takes it**, dialect-gated for the same two reasons: SQLite's first statement must be the write, so the guarded update carries the protection predicate as well; PostgreSQL takes the row and re-reads the protection as its own statement, which is what gives that read a snapshot taken after a claim already waiting on the row had committed. It costs lock duration — one Job's artifact deletion, bounded by the adapter — and buys the property the finding asked for, because a check before the deletion cannot be admitted under the same exclusion a claim is.
- **A republication clears the deferral.** The deferral is bookkeeping about one artifact row; an artifact produced again is a new one with its own deadline, so `upsertOutput` clears `next_cleanup_at` beside `removed_at`, and the deferral write is guarded on the row's own version so it cannot land on the row that replaced the selection.
- **`ReleaseClaim` is the release *and* the adapter's decided state, in one write.** A release that wrote only the token left a running Job that nothing could claim and nothing reconciles; the state therefore rides on the request and the whole thing is the transition machinery's own transaction, with `preparedTransition.releaseReason` keeping the runtime's reason on the claim row instead of the generic `state-changed`. A release under a token that owns nothing stays a no-op, which is what makes releasing idempotent and what keeps an abandoned runtime from taking a Job away from its successor.
- **`Finish` checks its own contract before it validates the transition.** `Finish` ends a Job, so a non-terminal outcome is `ErrInvalidTransition` rather than whichever lifecycle rule the shared validator would have reached first.

## Review

Every correction is at the seam its finding named, and the three hardest are pinned by interleavings rather than by statements about code: the cleanup fence by a claim issued while the pass is inside its deletion (on both engines, because the exclusion is dialect-gated), the fairness by a one-item batch over a protected artifact and over one no adapter can account for, and the release by a Job that leaves running and is claimed again by the next process. Tests that encoded the old behavior were corrected rather than weakened: the state machine's spec table and its expectation table, the two lifecycle helpers that walked into running by transition, `dispatch_pg_test.go`'s two release-then-requeue sequences (now one write), `query_pg_test.go`'s two walking helpers (now claims), and the release test whose premise — a release that moves neither state nor version — is exactly what the finding forbids.

Residual risks and handoffs:

- **The deletion holds the Job's row for the length of one Job's artifact cleanup.** On SQLite that is the deployment's write lock, so an adapter that blocks inside `CleanupArtifacts` holds other writers, pins and claims for that Job — the documented expectation that an adapter bounds its own cleanup is now load-bearing rather than a remark. The alternative, a durable cleanup reservation that `Claim` also consults, is a recovery protocol of its own and was not built.
- **A publication that lands between the selection and the fence is still deleted by key.** Pre-existing and unchanged: the Kind is asked about the artifacts the selection named, and the version guard keeps the *bookkeeping* right. The finding's fence closes the claim window, which is the window a new execution opens; closing this one needs the artifact reference to carry a version the Kind can confirm.
- **`SweepResult` does not report deferrals.** How many artifacts a pass left for later is visible on the rows (`next_cleanup_at`) and in the fact that a later pass reaches them; `ReconcileReport.Deferred` exists because a caller has no other way to ask, and this one does.
- **Test fixtures still write a running Job without a claim** (`seedJob`, and the new `startTestExecution`). They are setup for tests about something else, they say so, and the reachability they fabricate is now impossible through any public path — the state machine's table test and `TestJobRunningAdmissionIsTheClaimProtocol` are what pin that.
- **`JobRuntime.Stop` still writes nothing on the way out.** A Job an execution was still running keeps its state, its claim and its lease, and the next process reconciles it; the decided-state release is the path an adapter takes when it quiesces its own execution, not something shutdown does on its behalf.
...
- [x] Read the approved design (§2, §3, §5, §6, §7, §9), ADRs 0006/0007, the plan's Task 2/3/4/5 contracts, `CLAUDE.md`, and the current `jobs/`, `application_context/` and `models/` code.
- [x] Reproduce each finding as a red behavior test at the confirmed public seam before changing production code, including the two interleavings that only a second connection can express: a publication landing between a sweep's selection and its update, and a pin committing under an open transaction while the sweep waits.
- [x] Implement one minimal correction per finding, then re-run the focused, package-level, cross-engine and whole-tree suites.
- [x] `gofmt`, `go vet`, `git diff --check`, self-review of the whole diff, commit, clean worktree.
...
| Finding | Red (observed failure) | Correction and its test |
|---|---|---|
| A stale expiry scan can expire a newly republished output | `the republished artifact is expired, want it still available: the pass recorded an expiry for a row it did not select` — the republication is driven from the pass's own `UPDATE` statement on a second connection, so it lands exactly between the selection and the update | `expireOutputsOnTheirDeadline` guards each row it selected on that row's own `id`, `version` and deadline instead of on the id and availability alone (`TestRetentionOutputExpiryRechecksTheRowItSelected`) |
| PostgreSQL retention can delete a concurrently committed pin | `a Job pinned by a transaction that committed while the sweep waited was pruned anyway` — PostgreSQL, with the sweep observed waiting on the pin's row lock through `pg_stat_activity` | `pruneExpiredJob` takes the candidate's row (`SELECT ... FOR UPDATE`, dialect-gated) before the deciding statement, so the `NOT EXISTS (pins)` predicate is evaluated from a snapshot taken after the pin committed (`TestRetentionSweepCannotPruneAJobPinnedByAnOpenTransactionPG`) |
| Partial artifact cleanup leaves removed outputs advertised as available | `one of two artifacts is gone/artifact-gone is available, want removed: the cleanup acknowledged it gone`; the pin-landing-during-cleanup case failed the same way | `accountForArtifacts` records every acknowledged removal on its own row (guarded by that row's version) and appends one `output-removed` event, independently of whether the whole set went or the metadata may be pruned (`TestRetentionSweepRecordsEachAcknowledgedArtifactRemoval`, `EventOutputRemoved`) |
| Adapters can bypass the optional event ceiling | `the adapter's event phase-tick was stored as a reserved host fact`, `stored 406 adapter events, want the 400-event optional ceiling`, `recorded 0 truncation warnings` | `AppendEvent` — the path every adapter event takes — clears a caller-supplied `ReservedHost` instead of forwarding it, so reserved admission stays host-internal (`TestEventAppendKeepsTheReservedHeadroomHostInternal`) |
| A Kind capacity-group collision disables the global budget | `a second execution was admitted into the global budget: the Kind's declaration discarded the deployment-wide limit`, for both the zero-limit and the larger-limit Kind | Two limits for one group merge to the strictest *positive* one, so neither the deployment's ceiling nor a Kind's stricter cap is discarded (`TestKindCapacityGroupCollisionCannotDiscardTheDeploymentBudget`, three sub-cases) |
| Queued work with a missing adapter never becomes blocked | `svc.ReconcileUnrunnable undefined` at the module seam, and `timed out waiting for the Job to be blocked` through `JobRuntime.Start` with the pass unwired from `tick` (mutation check) | `ReconcileUnrunnable` blocks the bounded batch of unclaimed `queued`/`scheduled` Jobs whose Kind/version has no registration here, atomically through the same `Transition` every host-side change uses — no claim, no capacity (`TestReconcileUnrunnableBlocksPendingWorkNoAdapterCanRun`, `TestJobRuntimeBlocksQueuedWorkWhoseAdapterIsGone`) |
| Malformed decoded replay input leaves a claimed Job running without an executor | `state = running, want blocked` for all three shapes — empty, not JSON, over the ceiling | Post-decode validation failures are classified as `ErrReplayDecodeFailed`, which the dispatch seam already blocks on, so the claim and the capacity it took go with the block (`TestReplayPostDecodeValidationBlocksTheClaimAndReleasesItsCapacity`) |
| Replay codec failures can leak decrypted secrets into runtime logs | Restoring the raw text makes the module test quote `url-query-secret-8f1c`; through the runtime, the captured log line carries `runtime-url-query-secret-4d1a` and the whole decrypted payload | Codec boundaries (`Migrate`, `Decode`) return the classification and the Kind/version and never the codec's own text, which runs on decrypted bytes (`TestReplayCodecFailuresNeverQuoteTheDecryptedInput`, `TestJobRuntimeNeverLogsTheDecryptedInputItCouldNotDecode`) |
| Generated replay-key publication is not crash-durable | No directory flush existed to observe (`too many arguments in call`); with the flush dropped again: `flushed [], want the key file's own directory … once` and `a key whose name could not be made durable was accepted` | `publishReplayKeyFile` flushes the containing directory after the exclusive link — on the winning path and on the competing-publisher path that adopts another process's key — and refuses when it cannot, preserving the no-overwrite behaviour (`TestReplayKeyFilePublicationMakesTheNameDurable`, four sub-cases; the existing no-overwrite test now drives the real flush) |

## Verification

- `go test --tags 'json1 fts5' ./jobs -count=1` — passed.
- `go test --tags 'json1 fts5' ./jobs ./application_context ./internal/arch -count=1` — passed (`application_context` 61s, `internal/arch` 2.1s).
- `go test --tags 'json1 fts5 postgres' ./jobs -count=1` — passed, including the new pin-versus-prune regression.
- `go test --tags 'json1 fts5 postgres' ./jobs ./application_context -count=1` — passed.
- `go test --tags 'json1 fts5' ./... -count=1` — every package passed.
- Mutation checks (each defect is caught by the test written for it, and the source was restored): dropping the directory flush fails two `TestReplayKeyFilePublicationMakesTheNameDurable` sub-cases; restoring the codec error text fails both `TestReplayCodecFailuresNeverQuoteTheDecryptedInput` sub-cases; unwiring `ReconcileUnrunnable` from `tick` fails the runtime regression.
- `go vet --tags 'json1 fts5' ./...` — clean. `gofmt -l` on every changed file — clean. `git diff --check` — clean.
- No frontend source changed, so no bundle rebuild was needed.

## Decisions worth recording

- **The capacity-group collision is merged, not reserved.** The finding offered either. `CapacityGroup: "global"` is a legitimate spelling of "share the deployment-wide budget", and both limits in a collision are meaningful — the Kind's own cap and the deployment's ceiling — so taking either wholesale discards a real constraint. Merging to the strictest positive limit keeps both, and "0 means not enforced" is preserved rather than read as "unlimited".
- **`ReservedHost` stays on `EventInput`, but it is now host-internal in fact as well as in name.** `Transition` never read it (a lifecycle transition always records a reserved event) and `AppendEvent` no longer honours it, so the flag's only reader is the host's own recorders — which is what the doc comment claimed before the code agreed.
- **The replay flush is injected into `publishReplayKeyFile`.** The property worth pinning is the publication's *response* to a directory that cannot be flushed, and no real filesystem can be made to fail at that instant; the parameter is one function value on an unexported function with one production caller, in the shape the tree already uses for injected clocks and crypto randomness.

## Review

Every correction is at the seam its finding named, and the two hardest ones are pinned by interleavings rather than by statements about code: the expiry guard is exercised by a publication that lands inside the pass's own update, and the pin/prune ordering by a pin transaction that commits while the sweep is provably waiting on its row lock. The three corrections that touch durable state — the removal record, the pending-work block and the key-file publication — all keep the module's ordering invariants: the transaction's first statement is still a write on SQLite, the deciding statement still carries the whole predicate, and a refusal still writes nothing.

Residual risks and handoffs:

- **Reserved host events remain outside `MaxEventsPerJob`, deliberately.** §6's reservation is exactly that: host lifecycle and terminal facts cannot be displaced by adapter traffic. What the finding's correction changes is who may spend the headroom — before it, any adapter could — so "the ceiling is not enforced by any production write" is now a statement about adapter traffic rather than a hole. An adapter that loops claims and transitions can still add reserved lifecycle events at the rate the runtime grants it work; bounding that is a runtime-pacing question, not an event-capacity one.
- **`ReconcileUnrunnable` blocks pending work of a Kind *this process* has no adapter for**, and in a multi-process deployment that is a shared decision: a process that does not register a Kind blocks that Kind's waiting work for every process. That is ADR 0006's answer ("a missing adapter blocks pending work instead of falling back to another executor") and the finding's correction. It is not a dead end — `blocked → queued` is a legal transition, so an operator command (Task 6) or a later reconciliation can requeue it — but Task 10's single-fenced-command-runtime design must register the command Kinds in every process that can accept their work, or pending command imports will be blocked by the process that cannot run them.
- **A pinned Job is still never asked about its artifacts**, so an artifact that is gone while its Job is pinned stays advertised until the pin is released and the Job's window passes. That is the previous round's accepted behaviour ("a pin keeps everything it points at") and is asserted by an existing test; the new recording covers the pin that lands *during* a cleanup.
- **`expireOutputsOnTheirDeadline` now carries three binds per selected row**, so a pass at `MaxSweepBatch` (1000) issues a 3000-parameter statement — inside SQLite's 32766 and PostgreSQL's 65535 ceilings, and bounded by a batch the caller cannot exceed.
- **`lockPruneTarget` is dialect-gated on `tx.Dialector.Name()`**, like the pin-admission guard beside it: SQLite keeps its write-first statement and PostgreSQL takes the row. The PostgreSQL half is covered by a tagged regression; the SQLite half is covered by the whole SQLite suite, which cannot form the race.
- **The sweep still has no scheduler** (carried forward from Task 5): `ctx.SweepJobHistory` is a bounded call a loop or an operator drives, so the new removal recording runs when a sweep runs.

# Job Center checkpoint review corrections (GPT-6 Astra, after Task 5)

**Goal:** Correct every P0/P1 finding from the checkpoint review of Tasks 1-5 (durable lifecycle core, events/progress/links/outputs, encrypted replay, fenced dispatch, visibility/preferences/retention) without widening scope into Tasks 6-18.

## Plan

- [x] Read the approved design (§1, §3, §7, §8, §9, §10, §11), ADRs 0006/0007, the plan's Task 2/4/5 contracts, and the current `jobs/`, `application_context/` and `models/` code.
- [x] Reproduce each finding as a red behavior test at the confirmed seam before changing production code.
- [x] Implement one minimal correction per finding and re-run the focused, package and cross-engine suites.
- [x] `gofmt`, `go vet`, `git diff --check`, self-review of the whole diff, commit, clean worktree.

## Corrections (finding → red evidence → correction)

- **Success verification was only on `Finish`.** Red: `Transition(To: succeeded)` committed over an expired required output (`ErrRequiredOutputUnavailable` expected, got `nil`). Fixed by giving `Transition` the same `verifyRequiredOutputs` closure `Finish` uses; a Job with no required outputs still succeeds (`TestOutputSuccessThroughATransitionVerifiesRequiredOutputs`).
- **Lifecycle writes did not check the execution token atomically.** Red: a transition prepared before `ReleaseClaim` committed afterwards (release moves neither version nor state). Fixed by adding `COALESCE(execution_token,'') = ?` to the guarded UPDATE (`executionTokenMatch`, now one spelling for every executor-side write) (`TestFinishingIsFencedByTheClaimTheExecutionStillHolds`, also asserts the release-under-a-token-with-no-claim-row case).
- **Terminal event sequence was allocated outside the transaction.** Red: `AppendEvent` landing between decision and commit took the same position and the unique index rolled the completion back (`UNIQUE constraint failed: job_events.job_id, job_events.sequence`). Fixed by allocating inside `commitTransition` after the guarded write (`TestJobTerminalTransitionAllocatesItsEventSequenceInsideTheTransaction`).
- **A deleted actor silently became the owner or the host.** Red (mutation): after `DeleteUser` the Job was claimed and dispatched with `Access{UserID: 0}` — the host. Fixed with a durable `ExecutionPrincipal` class fixed at acceptance plus `executionAccess`, which refuses instead of substituting; dispatch blocks the Job (`execution_principal` column, `TestDeleteUser_RefusesDispatchOfAJobWhoseActorIsGone`, `blockedReasonPrincipalMissing`, quarantine for the reconcile path).
- **Visibility was not fixed by the registered Kind.** Red (mutation): an acceptance that omitted the class stored `owner`. Fixed with `Definition.Visibility` (validated, default `owner`) and `applyRegisteredVisibility`, which derives the class and refuses an acceptance that contradicts its Kind (`TestJobVisibilityIsFixedByTheRegisteredKind`).
- **Relationship filters revealed hidden lineage.** Red: a visible parent whose only child was admin-class matched the filter and was counted by `Summary`. Fixed by requiring the far endpoint to satisfy the shared visibility subquery (`TestListFiltersByLineageRelationshipWithoutRevealingAHiddenRelative`).
- **Ordinary snapshots carried the protected diagnostic reference.** Red: `Get` returned the internal path. Fixed with `viewerSnapshot`/`withoutDiagnosticRef` on every viewer-facing projection (Get, List, Lineage); administrators keep it (`TestProtectedDiagnosticsStayOutOfOrdinaryProjections`).
- **Heartbeats banked a lease each, accumulating recovery delay.** Red: an hour of heartbeats left the claim expiring 2h1m after the last one. Fixed by extending to `max(previous, now+lease)` in the statement; the old assertion that encoded the banking was corrected (`TestHeartbeatKeepsTheLeaseNearTheLastHeartbeatRatherThanAccumulatingIt`, `TestHeartbeatExtendsOnlyTheClaimItsTokenOwns`).
- **Stop waited for the reconciliation it had not yet cancelled.** Red: `Stop` never returned while an adapter waited on its context. Fixed by cancelling the lifecycle context first, bounding both drains, and binding the loop's database work to that context (`TestJobRuntimeStopsWhileReconciliationIsWaitingOnItsContext`).
- **Release/quarantine inverted the PostgreSQL lock order.** Red (PostgreSQL): `ERROR: deadlock detected (SQLSTATE 40P01)`. Fixed by taking the Job's row first in `releaseClaimTx` and `quarantineClaim` (`TestLifecycleReleaseAndQuarantineTakeTheJobLockFirstPG`).
- **Concurrent pins bypassed the per-user limit.** Red (PostgreSQL): an admission ran past a held guard. Fixed with the per-viewer `JobPinGuard` row taken and locked before the count (`TestPinAdmissionSerializesOnTheViewersGuardPG`).
- **Pinning could succeed after retention deleted the Job.** Red: an orphan pin row was created for a Job the sweep had just removed. Fixed by locking the Job's row and rechecking visibility inside the preference transaction (`TestPreferenceMutationRechecksTheJobItIsWrittenAgainst`, `TestPreferenceRowsGoWithTheJobTheyAreAbout`).
- **Output expiry was tied to metadata retention.** Red: two artifacts past their own deadline stayed `available` (`recorded 0 output availabilities`). Fixed with an independent bounded expiry pass in `Sweep` and a deadline check inside `verifyRequiredOutputs` (`TestRetentionSweepExpiresOutputsOnTheirOwnDeadline`, `TestOutputRequiredOutputsAreVerifiedAgainstTheirDeadlineNotOnlyTheStoredString`).
- **Artifact references were pruned without cleanup confirmation.** Red: the sweep pruned a Job whose artifact it never asked anyone about (`cleanup 0 times`). Fixed with `Adapter.CleanupArtifacts` as a bounded acknowledgement seam: a Job is pruned only when every artifact is confirmed gone, and a Job retention may not take at all (pinned, unresolved claim) is never asked (`TestRetentionSweepConfirmsArtifactCleanupBeforePruningHistory`).
- **Malformed replay nonces panicked.** Red: `panic: crypto/cipher: incorrect nonce length given to GCM`. Fixed by validating the nonce length before `Open` (`TestReplayMalformedStoredNoncesFailClosed`, through both `OpenReplay` and `Claim`).
- **Sealing could borrow another Kind version's codec.** Red: v1 acceptance succeeded with only a v2 codec registered. Fixed by requiring the exact `(kind, version)` codec for sealing while keeping the migration fallback for reading (`TestReplayAcceptanceRequiresTheExactKindVersionCodec`).
- **Replayable work could run without its envelope.** Red: acceptance with neither input nor an explicit class succeeded, and a replayable Job whose envelope was gone was claimed and dispatched. Fixed by requiring an envelope or an explicit non-replayable class at acceptance and by blocking replayable Jobs whose envelope is absent (`ErrReplayEnvelopeMissing`) (`TestReplayAcceptanceRequiresAnEnvelopeOrANonReplayableClass`, `TestReplayableJobWithoutItsEnvelopeIsBlockedRatherThanRun`).

## Review

48 acceptance literals across `jobs/` and `application_context/` tests were updated to the now-mandatory replay classification (`Replay: ReplayInput{NonReplayable: true}` for work with no input of its own); that is the only reason so many test files appear in the diff. Two existing assertions were corrected because they encoded the defects: the heartbeat test's banked expiry, and the post-terminal append in the truncation test, which is now refused outright (`ErrStaleExecution`) because finishing a Job ends the execution's ownership of it.

**Non-findings worth recording.** `ErrExecutionPrincipalUnavailable` is refused at claim time and quarantined on the reconcile path — quarantined rather than blocked because an expired claim cannot prove the external work stopped, which is the rule ADR 0006 states for a missing adapter. `CleanupArtifacts` runs with `context.Background()`: a sweep has no request to inherit a context from, and the call is bounded by the batch (one Job's artifacts, one cleanup per candidate). An adapter-less Kind keeps its artifact references — that is the finding's required behaviour, and it is why `TestRetentionSweepPrunesExpiredWorkOnPostgres` now registers the Kind it drives.
...
- [x] Task 4: Preserve the runtime-lease busy sentinel across the public boundary.
