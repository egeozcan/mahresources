...
- [ ] **Step 1: Add red tests for the declaration rules**
...
- [ ] **Step 3: Implement name validation and the limits**
...
- [ ] **Step 4: Parse the manifest field**
...
- [ ] **Step 4: Render it in both panel sites and the CLI**
...
- [ ] **Step 1: Add red `ValidateInputs` tests**
...
- [ ] **Step 2: Implement `ValidateInputs`**
...
- [ ] **Step 3: Add red Lua-surface tests**
...
- [ ] **Step 4: Implement the Lua surface**
...
- [ ] **Step 5: Plumb it through `Submit`**
...
- [ ] **Step 1: Red test — present before spawn, mode 0600 (spec §9.1)**
...
- [ ] **Step 2: Red tests — the failure and crash paths (spec §9.9, §9.10, §9.15, §9.16, §9.17)**
...
- [ ] **Step 3: Red test — queued cancellation writes nothing (spec §9.13)**

Cancel a queued run, drain, then assert the exchange folder has no supplied file and the run is `cancelled`.

- [ ] **Step 4: Red test — contents are dropped after the write (spec §9.18)**
...
- [ ] **Step 5: Implement the write step and wire it in**
...
- [ ] **Step 2: Extend the browser proof**
...
- [ ] **Step 3: Update the four docs pages**
...
- [ ] Spec §1 manifest rules map to Task 1 (grammar, dot rule, count, capability, identity-as-set, no argv-reference requirement).
- [ ] Spec §2 Lua surface maps to Tasks 3 and 4 (signature, strict keys, zero-byte, wording, transaction/disable refusals).
- [ ] Spec §3 ordering and atomicity maps to Task 4 (phase split, slot in `Execute`, stage/rename, verification, failure path, three consequences).
- [ ] Spec §4 limits and quota maps to Tasks 1, 3 and 4 (constants, synchronous aggregate, per-run quota admission, global sample refresh note).
- [ ] Spec §5 records and redaction maps to Tasks 4 and 5 (`InputsJSON` shape, no `ParamsJSON` key, tolerant decode, Lua view, admin table, `CommandDisplay.Inputs`, submit-time sizes).
- [ ] Spec §6 consent maps to Task 2 (grant, display, label phrases, both panel sites, CLI, docs).
- [ ] Spec §7 retention and secrets maps to Tasks 4 and 6 (queued cancellation writes nothing, worst-case docs sentence, discard/read-back).
- [ ] Spec §8 what must not change has no task that touches `Prepare`, `BuildInvocation`, `mah.fs` writes or the Windows refusal.
- [ ] Every test in spec §9 with a §9 number is named in Task 4 or Task 6; §9.7 and §9.14 are in Tasks 2, 3 and 5.
- [ ] `CONTEXT.md` terms (Command Declaration, Declared Input File, Run Parameter, Command Import, Exchange Folder) appear in the new docs prose, and "input file" is never used for a Command Import.
