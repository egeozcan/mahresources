{% if pagination != nil %}
<nav aria-label="Pagination" class="border-t border-gray-200 px-4 flex items-center justify-between sm:px-0 pb-2" data-has-prev="{% if pagination.PrevLink.Link != '' %}true{% else %}false{% endif %}" data-has-next="{% if pagination.NextLink.Link != '' %}true{% else %}false{% endif %}">
    <div class="-mt-px w-0 flex-1 flex {% if pagination.PrevLink.Link == '' %}invisible{% endif %}">
        {% if pagination.PrevLink.Link != '' %}
        <a href="{{ pagination.PrevLink.Link }}" data-pagination-prev class="border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
    ...
    <div class="-mt-px w-0 flex-1 flex justify-end {% if pagination.NextLink.Link == '' %}invisible{% endif %}">
        ...
        <a href="{{ pagination.NextLink.Link }}" data-pagination-next class="border-t-2 border-transparent pt-4 pl-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
