...
{% block head %}{% custom_css group %}{% endblock %}
...
{% block body %}
    ...
    <details class="detail-collapsible mb-6" {% if sc.OwnEntities.State == "open" %}open{% elif sc.OwnEntities.State == "collapsed" %}{% elif hasOwn %}open{% endif %}>
        ...
        <div class="detail-panel-body">
            ...
            {% include "/partials/seeAll.tpl" with entities=group.OwnResources subtitle="Resources" formAction="/resources" addAction="/resource/new" formID=group.ID formParamName="ownerId" templateName="resource" showUntaggedLink=true showReductionLink=true %}
            {% endif %}
            {% endif %}
        </div>
    </details>
...
{% block sidebar %}
    ...
    {% if group.GUID %}
    <div class="sidebar-group">
        <button type="button" class="text-xs text-stone-600 break-all cursor-pointer text-left bg-transparent border-0 p-0 w-full" title="Click to copy GUID" onclick="navigator.clipboard.writeText('{{ group.GUID }}')">
            GUID: {{ group.GUID }}
        </button>
    </div>
    {% endif %}
