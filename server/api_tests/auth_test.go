...
func setupAuthEnv(t *testing.T) *TestContext {
	...
	// Pin the test DB to a single connection.
	//
	// Neither reason it was added for applies any more. The harness DSN was
	// `cache=private`, where each new connection is a separate empty database, so a
	// concurrent token/session lookup could land on a fresh connection and
	// spuriously fail auth; it then became `cache=shared`, whose table locks raise
	// SQLITE_LOCKED without waiting on busy_timeout. It is a WAL file now (see
	// openTestDatabase), which has neither problem. The pin stays because unpinning
	// the seventeen tests that carry it is its own change and deserves its own
	// measurement, and it costs nothing here.
