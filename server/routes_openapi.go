...
func registerResourceRoutes(r *openapi.Registry) {
	...
	r.Register(openapi.RouteInfo{
		...
		Description:          "Uploads one or more files, or fetches a URL. `Headers` are extra request headers for a URL fetch, accepted on a JSON body only (a multipart or form post carries no header map); they are sent to the submitted URL's own origin and nowhere else, and connection-level headers, `Proxy-*`, `Keep-Alive` and `Range` are refused with HTTP 400.",
		Tags:                 []string{"resources"},
		HasFileUpload:        true,
