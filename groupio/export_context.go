...
func (ctx *opCtx) writeCategoryDefs(w *archive.Writer, plan *exportPlan) error {
	...
	for _, row := range rows {
		defs = append(defs, archive.CategoryDef{
			...
			CustomHeaderCSS:             row.CustomHeaderCSS,
			CustomSidebar:               row.CustomSidebar,
			CustomSidebarCSS:            row.CustomSidebarCSS,
			CustomSummary:               row.CustomSummary,
			CustomSummaryCSS:            row.CustomSummaryCSS,
			CustomAvatar:                row.CustomAvatar,
			CustomAvatarCSS:             row.CustomAvatarCSS,
			CustomListHeader:            row.CustomListHeader,
			CustomListHeaderCSS:         row.CustomListHeaderCSS,
			CustomDetailFooter:          row.CustomDetailFooter,
			CustomDetailFooterCSS:       row.CustomDetailFooterCSS,
			CustomListFooter:            row.CustomListFooter,
			CustomListFooterCSS:         row.CustomListFooterCSS,
			CustomHoverCard:             row.CustomHoverCard,
			CustomHoverCardCSS:          row.CustomHoverCardCSS,
			CustomOwnEntities:           row.CustomOwnEntities,
			CustomOwnEntitiesCSS:        row.CustomOwnEntitiesCSS,
			CustomEntityPickerResult:    row.CustomEntityPickerResult,
			CustomEntityPickerResultCSS: row.CustomEntityPickerResultCSS,
			CustomMRQLResult:            row.CustomMRQLResult,
			CustomMRQLResultCSS:         row.CustomMRQLResultCSS,
...
func (ctx *opCtx) writeResourceCategoryDefs(w *archive.Writer, plan *exportPlan) error {
	...
	for _, row := range rows {
		defs = append(defs, archive.ResourceCategoryDef{
			...
			CustomPreview:     row.CustomPreview,
			CustomPreviewCSS:  row.CustomPreviewCSS,
			CustomLightbox:    row.CustomLightbox,
			CustomLightboxCSS: row.CustomLightboxCSS,
			CustomCell:        row.CustomCell,
			CustomCellCSS:     row.CustomCellCSS,
		})
	}
